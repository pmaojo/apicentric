...
pub async fn load_service(
    ...
    match simulator.read_service_file(&request.path).await {
...
pub async fn save_service(
    ...
    match serde_yaml::from_str::<UnifiedConfig>(&request.yaml) {
        Ok(unified) => {
            let def = ServiceDefinition::from(unified);
            match serde_yaml::to_string(&def) {
                Ok(yaml) => match simulator.save_service_file(&request.path, &yaml).await {
...
pub async fn create_service(
    ...
    if simulator.service_file_exists(&filename).await {
...
pub async fn create_graphql_service(
    ...
    let services_dir_path = simulator.get_services_dir();
    let services_dir = services_dir_path.to_string_lossy();
