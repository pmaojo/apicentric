...
pub async fn import_from_url(
    ...
    if let Err(e) = crate::utils::security::validate_ssrf_url(&request.url).await {
        return Err(ApiError::bad_request(
            ApiErrorCode::InvalidParameter,
            format!("Invalid URL for import: {}", e),
        ));
    }

    // Create a client that doesn't follow redirects to prevent SSRF bypass
    let client = reqwest::Client::builder()
        .redirect(reqwest::redirect::Policy::none())
        .build()
        .map_err(|e| ApiError::internal_server_error(format!("Failed to create HTTP client: {}", e)))?;
