            ...
            Ok(unified) => {
                ...
                match serde_yaml::to_string(&def) {
                    ...
                    Err(e) => {
                        ...
                        Ok(Json(ApiResponse::error("Failed to process service".to_string())))
                    }
                }
            }
            Err(e) => {
                log::error!("Failed to parse YAML content: {}", e);
                Ok(Json(ApiResponse::error("Invalid service configuration".to_string())))
            }
        },
        Err(e) => {
            log::error!("Failed to read service file: {}", e);
            Ok(Json(ApiResponse::error("Failed to read service file".to_string())))
        }
    }
}
            ...
            match serde_yaml::to_string(&def) {
                Ok(yaml_content) => match simulator.save_service_file(&safe_path, &yaml_content) {
                    ...
                    Err(e) => {
                        ...
                        Ok(Json(ApiResponse::error("Failed to save service".to_string())))
                    }
                },
                Err(e) => {
                    log::error!("Failed to serialize service definition: {}", e);
...
pub async fn reload_services(
    ...
    match simulator.reload_services().await {
        ...
        Err(e) => {
            ...
            Ok(Json(ApiResponse::error("Failed to reload services".to_string())))
        }
    }
}
