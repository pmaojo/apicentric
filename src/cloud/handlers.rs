...
pub async fn load_service(
    ...
    let safe_path = match simulator.resolve_service_path(&request.path) {
        ...
        Err(e) => {
            ...
            return Ok(Json(ApiResponse::error("Invalid service path".to_string())));
        }
    };
                ...
                match serde_yaml::to_string(&def) {
                    ...
                    Err(e) => {
                        ...
                        Ok(Json(ApiResponse::error(
                            "Failed to process service definition".to_string(),
                        )))
                    }
                }
            }
            Err(e) => {
                log::error!("Invalid YAML in service file: {}", e);
                Ok(Json(ApiResponse::error(
                    "Invalid YAML format in service file".to_string(),
                )))
            }
        },
        Err(e) => {
            ...
            Ok(Json(ApiResponse::error(
                "Failed to read service file".to_string(),
            )))
        }
    }
}
...
pub async fn save_service(
    ...
    match serde_yaml::from_str::<UnifiedConfig>(&request.yaml) {
            ...
            match serde_yaml::to_string(&def) {
                Ok(yaml_content) => match simulator.save_service_file(&safe_path, &yaml_content) {
                    ...
                    Err(e) => {
                        ...
                        Ok(Json(ApiResponse::error(
                            "Failed to save service file".to_string(),
                        )))
                    }
                },
                Err(e) => {
        ...
        Err(e) => {
            log::error!("Invalid YAML provided for service definition: {}", e);
            Ok(Json(ApiResponse::error("Invalid YAML format".to_string())))
        }
    }
}
