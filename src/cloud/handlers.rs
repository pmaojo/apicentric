...
pub async fn update_config(
    ...
    // Load current configuration to handle redacted secrets
    // Attempt to load current config, or use default if it doesn't exist
    // This allows merge_with_current to work even if we're creating the file for the first time (though in that case there are no secrets to restore)
    let current_config = load_config(Path::new(&config_path)).unwrap_or_default();

    // Restore redacted fields from current configuration
