...
pub async fn save_service(
    ...
    match serde_yaml::from_str::<UnifiedConfig>(&request.yaml) {
        Ok(unified) => {
            let def = ServiceDefinition::from(unified);
            match serde_yaml::to_string(&def) {
                Ok(yaml_content) => match simulator.save_service_file(&request.path, &yaml_content)
                {
...
pub async fn create_service(
    ...
    match simulator.service_file_exists(&filename) {
        Ok(exists) => {
            if exists {
...
pub async fn create_graphql_service(
    ...
    // Get services directory path for the YAML content
...
#[cfg(test)]
mod tests {
    // Tests for resolve_safe_service_path removed as the function was moved to ConfigFileLoader
}
