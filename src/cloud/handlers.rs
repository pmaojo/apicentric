...
pub async fn load_service(
    ...
    // Extract filename to ensure we are looking for the right file
    // We maintain flattening behavior for compatibility/security
    ...
    match simulator.read_service_file(filename).await {
...
pub async fn save_service(
    ...
    // Flatten filename for safety/compatibility
    let filename = std::path::Path::new(&request.path)
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or(&request.path)
    ...
    match serde_yaml::from_str::<UnifiedConfig>(&request.yaml) {
        Ok(unified) => {
            let def = ServiceDefinition::from(unified);
            match serde_yaml::to_string(&def) {
                Ok(yaml_str) => match simulator.save_service_file(filename, yaml_str).await {
...
pub async fn create_service(
    ...
    // Check if file already exists via simulator
    if simulator.service_file_exists(filename.clone()).await {
        return Err(ErrorResponse::service_already_exists(&filename).into());
    }

    // Write the service definition to file via simulator
    match simulator
        .save_service_file(filename.clone(), request.yaml)
        .await
    {
        Ok(_) => {
            // Apply the service to the running simulator
            if let Err(e) = simulator.apply_service_definition(definition).await {
                // Clean up the file if applying fails via simulator
                let _ = simulator.delete_service_file(filename).await;
...
pub async fn create_graphql_service(
    ...
    let services_dir_str = services_dir.to_string_lossy();
...
graphql:
  ...
  mocks:
    helloQuery: {}/{}"#,
        request.name, request.port, services_dir_str, schema_filename, services_dir_str, mock_filename
    );

    // Write files via simulator
    simulator
        .save_service_file(schema_filename.clone(), "type Query {\n  hello: String\n}".to_string())
        .await
    ...
    simulator
        .save_service_file(service_filename.clone(), yaml_content.clone())
        .await
...
pub async fn update_service(
    ...
    match simulator.save_service_file(filename, request.yaml).await {
        Ok(_) => {
...
pub async fn delete_service(
    ...
    match simulator.delete_service_file(filename).await {
        Ok(_) => Ok(Json(ApiResponse::success(format!(
