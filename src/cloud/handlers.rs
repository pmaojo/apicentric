    ...
    let safe_path = match simulator.resolve_service_path(&request.path) {
        ...
        Err(e) => {
            ...
            return Ok(Json(ApiResponse::error(
                "Failed to resolve service path".to_string(),
            )));
        }
    };
                ...
                match serde_yaml::to_string(&def) {
                    ...
                    Err(e) => {
                        ...
                        Ok(Json(ApiResponse::error(
                            "Failed to serialize service definition".to_string(),
                        )))
                    }
                }
            }
            Err(e) => {
                log::error!("Failed to parse service definition: {}", e);
                Ok(Json(ApiResponse::error(
                    "Failed to parse service definition".to_string(),
                )))
            }
        },
        Err(e) => {
