...
use crate::cli::args::{AiAction, Cli, CliExecutionMode, Commands, ExportFormat, SimulatorAction};
#[cfg(feature = "iot")]
...
fn print_help() {
    println!(
        r#"Usage: apicentric [OPTIONS] <COMMAND>
    ...
    while let Some(arg) = iter.peek() {
        ...
        match arg.as_str() {
            "--config" | "-c" => {
                cli.config = iter
                    .next()
                    .ok_or(ParseError::MissingArgument("--config".into()))?
                    .clone();
            }
            "--mode" => {
                let mode_str = iter
                    .next()
                    .ok_or(ParseError::MissingArgument("--mode".into()))?;
            ...
            "--db-path" => {
                cli.db_path = iter
                    .next()
                    .ok_or(ParseError::MissingArgument("--db-path".into()))?
                    .clone();
            }
            "--help" | "-h" => {
                return Err(ParseError::HelpRequested("Full help text here...".into()))
            } // Simplified
    ...
    if let Some(cmd) = iter.next() {
        match cmd.as_str() {
            ...
            "new" => {
                let name = iter
                    .next()
                    .ok_or(ParseError::MissingArgument("name".into()))?
                    .clone();
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        "--template" | "-t" => {
                            template = Some(
                                iter.next()
                                    .ok_or(ParseError::MissingArgument("--template".into()))?
                                    .clone(),
                            );
                        }
                        _ => return Err(ParseError::UnknownArgument(arg.clone())),
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        "--port" | "-p" => {
                            let p = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--port".into()))?;
                            port = Some(p.parse().map_err(|_| {
                                ParseError::InvalidValue("--port".into(), p.clone())
                            })?);
                        }
                        _ => return Err(ParseError::UnknownArgument(arg.clone())),
                    }
    ...
    if let Some(action) = iter.next() {
        match action.as_str() {
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        "--services-dir" | "-s" => {
                            services_dir = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--services-dir".into()))?
                                .clone()
                        }
                        "--force" => force = true,
                        "--template" => {
            ...
            "validate" | "v" => {
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        "--file" | "--path" => {
                            file = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--file".into()))?
                                .clone()
                        }
                ...
                Ok(Some(SimulatorAction::Validate {
                    file,
                    recursive,
                    verbose,
                }))
            ...
            "logs" | "l" => {
                let service = iter
                    .next()
                    .ok_or(ParseError::MissingArgument("service".into()))?
                    .clone();
                let mut limit = 20;
                let mut method = None;
                let mut route = None;
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        "--limit" | "-l" => {
                            let l = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--limit".into()))?;
                            limit = l.parse().map_err(|_| {
                                ParseError::InvalidValue("--limit".into(), l.clone())
                            })?;
                        }
                        "--method" => {
                            method = Some(
                                iter.next()
                                    .ok_or(ParseError::MissingArgument("--method".into()))?
                                    .clone(),
                            )
                        }
                        "--route" => {
                            route = Some(
                                iter.next()
                                    .ok_or(ParseError::MissingArgument("--route".into()))?
                                    .clone(),
                            )
                        }
                        "--status" => {
                            let s = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--status".into()))?;
                            status = Some(s.parse().map_err(|_| {
                                ParseError::InvalidValue("--status".into(), s.clone())
                            })?);
                        }
                        "--output" => {
                            output = Some(
                                iter.next()
                                    .ok_or(ParseError::MissingArgument("--output".into()))?
                                    .clone(),
                            )
                        }
                        _ => return Err(ParseError::UnknownArgument(arg.clone())),
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        ...
                        "--interval" => {
                            let i = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--interval".into()))?;
                            interval = Some(i.parse().map_err(|_| {
                                ParseError::InvalidValue("--interval".into(), i.clone())
                            })?);
                        }
                        _ => return Err(ParseError::UnknownArgument(arg.clone())),
                    }
                }
                Ok(Some(SimulatorAction::Monitor {
                    service,
                    json,
                    interval,
                }))
            ...
            "set-scenario" => {
                let scenario = iter
                    .next()
                    .ok_or(ParseError::MissingArgument("scenario".into()))?
                    .clone();
            ...
            "import" => {
                ...
                if file.is_empty() {
                    return Err(ParseError::MissingArgument("--file".into()));
                }
                if output.is_empty() {
                    return Err(ParseError::MissingArgument("--output".into()));
                }
            ...
            "export" => {
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        ...
                        "--format" | "-f" => {
                            let f = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--format".into()))?;
                ...
                Ok(Some(SimulatorAction::Export {
                    file,
                    output,
                    format,
                }))
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        ...
                        "--url" => {
                            url = Some(
                                iter.next()
                                    .ok_or(ParseError::MissingArgument("--url".into()))?
                                    .clone(),
                            )
                        }
                        _ => return Err(ParseError::UnknownArgument(arg.clone())),
            ...
            "dockerize" => {
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        "--services" | "--file" | "-f" | "-s" => {
                            ...
                            files.push(
                                iter.next()
                                    .ok_or(ParseError::MissingArgument("--services".into()))?
                                    .clone(),
                            );
                        }
                        "--output" | "-o" => {
                ...
                if files.is_empty() {
                    return Err(ParseError::MissingArgument("--services".into()));
                }
                Ok(Some(SimulatorAction::Dockerize {
                    file: files,
                    output,
                }))
            }
            ...
            "test" => {
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        "--path" | "-p" => {
                            path = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--path".into()))?
                                .clone()
                        }
                        "--url" | "-u" => {
                            url = iter
                        ...
                        "--env" => {
                            env = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--env".into()))?
                                .clone()
                        }
                        "--quiet" | "-q" => quiet = true,
                        _ => return Err(ParseError::UnknownArgument(arg.clone())),
                ...
                if url.is_empty() {
                    return Err(ParseError::MissingArgument("--url".into()));
                }
                Ok(Some(SimulatorAction::Test {
                    path,
                    url,
                    env,
                    quiet,
    ...
    if let Some(action) = iter.next() {
        match action.as_str() {
            "generate" => {
                let prompt = iter
                    .next()
                    .ok_or(ParseError::MissingArgument("prompt".into()))?
                    .clone();
    ...
    if let Some(cmd) = iter.next() {
        match cmd.as_str() {
            "run" => {
                ...
                while let Some(arg) = iter.next() {
                    match arg.as_str() {
                        "--device" | "-d" => {
                            device = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--device".into()))?
                                .clone()
                        }
                        "--override-config" => {
                            override_config = Some(
                                iter.next()
                                    .ok_or(ParseError::MissingArgument("--override-config".into()))?
                                    .clone(),
                            )
                        }
                        "--library" => {
                            library = iter
                                .next()
                                .ok_or(ParseError::MissingArgument("--library".into()))?
                                .clone()
                        }
                        _ => {
                ...
                Ok(TwinCommands::Run(TwinRunArgs {
                    device,
                    override_config,
                    library,
                }))
...
mod tests {
    ...
    fn test_simulator_start() {
        let args = to_args("simulator start --services-dir ./myservices --force");
        let cli = parse_args(&args).unwrap();
        match cli.command {
            Commands::Simulator { action } => match action.unwrap() {
