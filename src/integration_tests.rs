...
#[cfg(test)]
mod tests {
    use crate::adapters::npm::NpmConfig;
    use crate::adapters::npm_integration::NpmIntegration;
    use crate::adapters::server_manager::{MockServerManager, ServerManager, ServerManagerPort};
    use crate::config::{
        generate_default_config, load_config, save_config, ExecutionConfig, ExecutionMode,
        PulseConfig, ServerConfig,
    };
    use crate::errors::{ErrorFormatter, PulseError};
    ...
    fn test_complete_config_workflow() {
        ...
        assert_eq!(
            loaded_config.server.start_command,
  ...
  "scripts": {
    ...
    fn test_npm_workspace_detection() {
        ...
        fs::write(
            utils_mockforge.join("Cargo.toml"),
            "[package]\nname = \"mockforge\"",
        )
        .unwrap();
