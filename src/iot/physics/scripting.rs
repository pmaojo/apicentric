        ...
        match result {
            Ok(val) => {
                let var_val = if val.is_int() {
                    VariableValue::Integer(val.as_int().unwrap_or_default())
                } else if val.is_float() {
                    VariableValue::Float(val.as_float().unwrap_or_default())
                } else if val.is_bool() {
                    VariableValue::Boolean(val.as_bool().unwrap_or_default())
                } else if val.is_string() {
                    VariableValue::String(val.into_string().unwrap_or_default())
                } else if val.is_map() {
