...
use mockforge::{PulseError, PulseResult};
...
pub fn tui_command() -> PulseResult<()> {
    enable_raw_mode().map_err(|e| {
        PulseError::runtime_error(format!("Failed to enable raw mode: {e}"), None::<String>)
    })?;
    ...
    disable_raw_mode().map_err(|e| {
        PulseError::runtime_error(format!("Failed to disable raw mode: {e}"), None::<String>)
    })?;
    let mut stdout = io::stdout();
    execute!(stdout, LeaveAlternateScreen, DisableMouseCapture).map_err(|e| {
        PulseError::runtime_error(format!("Failed to restore terminal: {e}"), None::<String>)
    })?;
    terminal.show_cursor().map_err(|e| {
        PulseError::runtime_error(format!("Failed to show cursor: {e}"), None::<String>)
    })?;

    res
}

fn run_app<B: Backend>(terminal: &mut Terminal<B>) -> PulseResult<()> {
    loop {
        terminal
            .draw(|f| {
                let size = f.size();
            ...
            .map_err(|e| PulseError::runtime_error(format!("Render error: {e}"), None::<String>))?;

        if event::poll(Duration::from_millis(250)).map_err(|e| {
            PulseError::runtime_error(format!("Event poll failed: {e}"), None::<String>)
        })? {
            match event::read().map_err(|e| {
                PulseError::runtime_error(format!("Event read failed: {e}"), None::<String>)
            })? {
