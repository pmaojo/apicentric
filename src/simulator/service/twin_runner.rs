        ...
        let server_handle = tokio::spawn(async move {
            ...
            for transport in &twin_def.transports {
                match transport.adapter_type.as_str() {
                    ...
                    _ => error!(target: "simulator", "Unknown transport type: {}", transport.adapter_type),
                }
            }
                        ...
                        if let Some(script_val) = physics.params.get("code") {
                            if let Some(script) = script_val.as_str() {
                                match RhaiScriptStrategy::new(script, physics.variable.clone()) {
                                    ...
                                    Err(e) => error!(target: "simulator", "Failed to init Rhai strategy: {}", e),
                                }
                            }
                        }
                    }
                    "sine" | "noise_sine" => {
                        let min = physics.params.get("min").and_then(|v| v.as_f64()).unwrap_or(0.0);
                        let max = physics.params.get("max").and_then(|v| v.as_f64()).unwrap_or(100.0);
                        let frequency = 0.1;
                        let strategy = SineWaveStrategy::new(physics.variable.clone(), min, max, frequency);
                        strategies.push(Box::new(strategy));
                    }
                    _ => error!(target: "simulator", "Unknown physics strategy: {}", physics.strategy),
                }
            }
