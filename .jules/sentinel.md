...
## 2025-02-28 - [Information Exposure]
**Vulnerability:** In `load_service`, `save_service`, and `reload_services` handlers, raw internal file loading and parsing errors generated by backend components were being converted to strings and returned directly to users within HTTP JSON responses. This leads to an Information Exposure vulnerability (CWE-209), where internal server file paths or schema structures are leaked.
**Learning:** Returning `e.to_string()` directly in API handlers is unsafe if the underlying error is a filesystem or serialization error that embed internal path/state details.
**Prevention:** Wrap these handlers' errors using `log::error!` for internal observability, and return a sanitized, generic message (e.g. "Failed to load service") back to the user via `ApiResponse::error`.
