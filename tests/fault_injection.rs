...
",
    ...
    cmd.assert().failure().stderr(
        predicate::str::contains("Could not detect OpenAPI version")
...
fn test_start_missing_service_file() {
    ...
    cmd.assert().failure().stderr(
        predicate::str::contains("not found").or(predicate::str::contains("Template error")),
    );
}
...
fn test_cli_unknown_argument() {
    let mut cmd = Command::cargo_bin("apicentric").unwrap();
    cmd.arg("simulator").arg("status").arg("--what-is-this");

    cmd.assert().failure().stderr(
        predicate::str::contains("Unknown argument")
...
fn test_dockerize_missing_files() {
    ...
    cmd.assert().failure().stderr(
        predicate::str::contains("File system error")
...
fn test_twin_run_missing_device() {
    let mut cmd = Command::cargo_bin("apicentric").unwrap();
    cmd.arg("twin").arg("run").arg("non_existent_device");

    cmd.assert().failure().stderr(
        predicate::str::contains("File system error").or(predicate::str::contains("Runtime error")),
    );
}
