...
#[tokio::test]
async fn test_save_twin_path_traversal() {
    // Setup
    let temp_iot_dir = "temp_iot_traversal_test";
...
twin:
  ...
  transports: []
"#;
    let request = SaveTwinRequest { yaml: yaml.to_string() };
    ...
    if target_pwned_path.exists() {
        ...
        let _ = fs::remove_file(&target_pwned_path);
        let _ = fs::remove_dir_all(temp_iot_dir);
        panic!("Vulnerability STILL EXIST: File created at {:?}", target_pwned_path);
    }
    ...
    println!("Security Fix Verified: File created at {:?}", sanitized_path);
...
#[tokio::test]
async fn test_delete_twin_path_traversal() {
     // Setup
    let temp_iot_dir = "temp_iot_delete_traversal_test";
    let target_dir = "target";
    ...
    if !target_victim_path.exists() {
         ...
         fs::write(&target_victim_path, "important data").unwrap();
         panic!("Vulnerability STILL EXIST: File deleted at {:?}", target_victim_path);
    }
    ...
    if sanitized_path.exists() {
        ...
        let _ = fs::remove_dir_all(temp_iot_dir);
        panic!("Sanitization failed: Decoy file NOT deleted at {:?}", sanitized_path);
    }
