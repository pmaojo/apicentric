...
async fn test_path_traversal_prevention() {
    ...
    // Set env var to point to our temp services dir (though manager config takes precedence now)
    std::env::set_var("APICENTRIC_SERVICES_DIR", services_dir.to_str().unwrap());

    // Initialize Simulator Manager with the secure services directory
    ...
    let result = load_service(State(simulator.clone()), Json(request)).await;
    ...
    let result = save_service(State(simulator.clone()), Json(request)).await;
    if let Ok(Json(response)) = result {
